...
class DeepAtmosphere {
  ...
  generateStars() {
    this.stars = [];
    // Ultra-fine shrunken pinpricks for true galaxy depth
  ...
  updateComet(deltaTime) {
    ...
    // Perfect 4-second loop cycle
    if (this.comet.timer >= 4000) {
      this.comet.timer = 0;
      this.comet.active = true;
      // Start diagonally top-left-ish, off-screen
      this.comet.x = Math.random() * (this.canvas.width * 0.5);
      this.comet.y = -50;
    }

    if (this.comet.active) {
      // Speed tuned to cross cleanly within its window
      this.comet.x += 8;
      this.comet.y += 6;
  ...
  draw() {
    ...
    // Render Kinetic Comet
    if (this.comet.active) {
      let gradient = this.ctx.createLinearGradient(
        this.comet.x, this.comet.y, 
        this.comet.x - 60, this.comet.y - 45
      );
      gradient.addColorStop(0, 'rgba(253, 245, 230, 1)'); // Light Gold head
      gradient.addColorStop(0.2, 'rgba(179, 157, 219, 0.4)'); // Light Purple mid
